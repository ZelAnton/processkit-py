...
class Command:
    ...
    def arg0(self, arg0: str) -> Command:
    ...
    def idle_timeout(self, seconds: float) -> Command:
    ...
    def stdout_raw_tee(self, sink: StrPath | SupportsWriteBytes, *, append: bool = ...) -> Command:
    ...
    def stderr_raw_tee(self, sink: StrPath | SupportsWriteBytes, *, append: bool = ...) -> Command:
    ...
    def merge_stderr_in_pipe(self) -> Command:
    ...
    @property
    def configured_arg0(self) -> str | None:
...
class ProcessStdin:
    ...
    def write(self, data: ReadableBuffer) -> Awaitable[None]: ...
    def write_line(self, line: str) -> Awaitable[None]: ...
...
class RunningProcess:
    ...
    **consumes** the handle — afterwards it is spent (``pid`` and the other
    ...
    @property
    def stdout_bytes_seen(self) -> int | None:
    ...
    @property
    def stderr_bytes_seen(self) -> int | None:
    ...
    # These dunders return the runtime's PyLazyFuture, like every other async
    # binding. async with accepts an Awaitable, so do not model them as native
    # coroutines (which asyncio.run() / create_task() would incorrectly accept).
    def __aenter__(self) -> Awaitable[RunningProcess]: ...
    def __aexit__(
        self,
        exc_type: type[BaseException] | None = ...,
        exc_value: BaseException | None = ...,
    ...
    def lifecycle_events(self) -> LifecycleEvents:
        ...
        """
    def wait_for_output(self, predicate: str | Callable[[str], bool], *, timeout: float) -> str:
    ...
    def await_for_output(
        self, predicate: str | Callable[[str], bool], *, timeout: float
    ) -> Awaitable[str]:
...
class ProcessGroup(_RunnerVerbs):
    ...
    # Match RunningProcess: PyLazyFuture is an awaitable accepted by async with,
    # but it is not a native coroutine.
    def __aenter__(self) -> Awaitable[ProcessGroup]: ...
    def __aexit__(
        self,
        exc_type: type[BaseException] | None = ...,
        exc_value: BaseException | None = ...,
...
class IdleTimeout(ProcessError):
