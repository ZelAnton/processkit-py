...
def test_cancelling_awaited_run_kills_tree(pid_file: pathlib.Path) -> None:
...
def test_async_group_teardown_kills_grandchild(pid_file: pathlib.Path) -> None:
    async def scenario() -> int:
        async with ProcessGroup() as group:
            assert group.mechanism in {"job_object", "cgroup_v2", "process_group"}
            await group.astart(spawn_grandchild_command(pid_file))
            grandchild_pid = await asyncio.to_thread(read_pid_when_ready, pid_file, 10.0)
            assert is_alive(grandchild_pid)
            return grandchild_pid
...
def test_explicit_ashutdown_reaps_tree(pid_file: pathlib.Path) -> None:
...
def test_wait_for_timeout_kills_tree(pid_file: pathlib.Path) -> None:
