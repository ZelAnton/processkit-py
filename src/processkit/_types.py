...
canonical spelling is what type checkers surface. `ReadableBuffer` is what
`Command.stdin_bytes()` / `ProcessStdin.write()` accept — `bytes` and every
other object PyO3 extracts a byte buffer from via the buffer protocol
...
can `from processkit import (Args, LineTerminatorName, ReadableBuffer,
RetryIf, SignalName, StrPath)`.
"""
